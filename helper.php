...
function logInEmploy($idUser, $pass ,$connect){
	...
	if(!empty($idUser) && !empty($pass)){
		...
		$result= mysql_fetch_array(mysqli_query($sql));
		if ($result)
		{
			$isLogIn=true;
...
function getPlan($idPlan, $connect)
{
	...
	if(!empty($idPlan))
	{
		...
		$result = mysql_fetch_array(mysqli_query($sql,$connect));
		if ($result)
		{
...
function getDataCobro($connect, $idUser, $idFolio)
{
	...
	$result = mysql_fetch_array(mysqli_query($sql2,$connect));
...
{	
	...
	if (mysql_num_rows(mysqli_query($sql,$connect))>0)
	{
